...
def _lms_api_key(repo="."):
    """Resolve the LMS-TMS service key: env, then project-level, then user-level.
    The API requires it in the x-api-key header; without it every call 401s."""
    import re as _re
    ...
    for path in (os.path.join(repo, ".claude", "lms_tms_api_key"),
        ...
        if path.endswith(".env"):
            m = _re.search(r"^\s*(?:export\s+)?LMS_TMS_API_KEY\s*=\s*['\"]?([^'\"\n#]+)", text, _re.M)
            if m:
...
def get_json(url):
    try:
        _h = dict(UA); _k = _lms_api_key()
        if _k: _h["x-api-key"] = _k
        with urllib.request.urlopen(urllib.request.Request(url, headers=_h), timeout=60) as r:
